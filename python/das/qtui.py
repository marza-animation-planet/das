...
if not NoUI:
   ...
   class ListFilter(FieldFilter):
      def __init__(self, name, values, partial=False, invert=False):
         super().__init__(name)
         self.values = set([str(x) for x in values])
         self.partial = partial
         self.invert = invert
   ...
   class FilterSet(FieldFilter):
      ...
      def copy(self):
         ...
         rv.filters = [x.copy() for x in self.filters]
         return rv
   ...
   class NewValueDialog(QtWidgets.QDialog):
      def __init__(self, vtype, excludes=None, name=None, parent=None):
         super().__init__(parent, QtCore.Qt.WindowTitleHint|QtCore.Qt.WindowSystemMenuHint)
   ...
   class Model(QtCore.QAbstractItemModel):
      ...
      def __init__(self, data, type=None, name=None, readonly=False, headers=None, fieldfilters=None, parent=None):
         ...
         if headers is None:
            ...
            hdrs = [x for x in headers if x in self.AllHeaders]
            if not "Name" in hdrs:
               hdrs.insert(0, "Name")
   ...
   class Editor(QtWidgets.QTreeView):
      ...
      def keyPressEvent(self, event):
         if (event.modifiers() & QtCore.Qt.ControlModifier) != 0:
            ...
            if event.key() == QtCore.Qt.Key_Delete:
               ...
               if len({"" if x[1].parent is None else x[1].parent.fullname() for x in iipairs}) == 1:
      ...
      def mousePressEvent(self, event):
         if event.button() == QtCore.Qt.RightButton:
            ...
            if len({"" if x[1].parent is None else x[1].parent.fullname() for x in iipairs}):
               # All items have same parent
               index, item = iipairs[0]
               ...
               def type_str(t):
                  if isinstance(t, das.schematypes.SchemaType):
                     ...
                     return "tuple (%s)" % ", ".join([type_str(x) for x in t.types])
                     ...
                     if item.mapping:
                        if item.mappingkeytype is not None:
                           ...
                           if isinstance(item.type.vtype, das.schematypes.Or):
                              vtypes = [x for x in item.type.vtype.types if not isinstance(x, das.schematypes.Deprecated)]
                           if len(vtypes) > 1:
                        ...
                        if item.orderable:
                           if item.resizable:
                              ...
                              if isinstance(item.type.type, das.schematypes.Or):
                                 vtypes = [x for x in item.type.type.types if not isinstance(x, das.schematypes.Deprecated)]
                              if len(vtypes) > 1:
                                 actionAddMenu = menu.addMenu("Add to '%s'" % ifn)
                                 for ot in vtypes:
                                    actionAddItem = actionAddMenu.addAction(type_str(ot))
      ...
      def remSeqItems(self, indices):
         # Not really
         if len(indices) > 0:
            parents = set(["" if x.parent else x.parent.fullname() for x in [y.internalPointer() for y in indices]])
            if len(parents) != 1:
               return
